 ...
 * in global memory.
 *
 * @args list_m : int32_t* , a big array with N/9 matrices. The 'matrices' are
 *                stored as plain numbers in one big array.
 *       N : int32_t, the number of matrices in mats
 *
 * @return
 */
__global__ void reduce_min( int32_t *list_m, int32_t N ) {
 ...
 * matrices. It then transfers the matrices to the GPU, calls the Kernel to reduce
